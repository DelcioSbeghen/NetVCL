...
{**************************************************************************************************}
{                                                                                                  }
{ Last modified: $Date:: 2016-09-17 16:20:56 +0200                                               $ }
{ Revision:      $Rev:: 333b06de50f3a99c2c7f1adc3d9d8edf8d3950d1                                 $ }
{ Author:        $Author:: Jean-Fabien Connault                                                  $ }
{                                                                                                  }
{**************************************************************************************************}
...
{$IFDEF MSWINDOWS}
//DELCIO function CreateEnvironmentBlock(const Options: TEnvironmentOptions; const AdditionalVars: TStrings): PChar;
procedure DestroyEnvironmentBlock(var Env: PChar);
//DELCIO procedure SetGlobalEnvironmentVariable(VariableName, VariableContent: string);
{$ENDIF MSWINDOWS}

// Common Folder Locations
{$IFDEF MSWINDOWS}
//DELCIO function GetCommonFilesFolder: string;
{$ENDIF MSWINDOWS}
function GetCurrentFolder: string;
{$IFDEF MSWINDOWS}
//DELCIO function GetProgramFilesFolder: string;
(* DELCIO function GetWindowsFolder: string;
function GetWindowsSystemFolder: string;
function GetWindowsTempFolder: string;

//DELCIO function GetDesktopFolder: string;
...
{$IFDEF MSWINDOWS}
(* DELCIO function GetRegisteredCompany: string;
...
//DELCIO function GetOpenGLVersion(const Win: THandle; out Version, Vendor: AnsiString): Boolean;
...
// Hardware
{$IFDEF MSWINDOWS}
// DELCIO function GetMacAddresses(const Machine: string; const Addresses: TStrings): Integer;
{$ENDIF MSWINDOWS}
...
const
  UnitVersioning: TUnitVersionInfo = (
    RCSfile: '$URL: jcl/source/common/JclSysInfo.pas $';
    Revision: '$Revision: 333b06de50f3a99c2c7f1adc3d9d8edf8d3950d1 $';
    Date: '$Date: 2016-09-17 16:20:56 +0200 $';
...
uses
  ...
  {$IFDEF MSWINDOWS}
  Winapi.Messages, Winapi.Winsock, {DELCIO Snmp,}
  {$IFDEF FPC}
 ...
 {DELCIO JclShell,}
  ...
  {$ENDIF MSWINDOWS}
  {$ENDIF ~HAS_UNITSCOPE}
  {DELCIO Jcl8087, JclIniFiles,}
  NVJclSysUtils, NVJclFileUtils, NVJclAnsiStrings, NVJclStrings, Registry;
...
function ExpandEnvironmentVarCustom(var Value: string; Vars: TStrings): Boolean;
  ...
  begin
    ...
    begin
      ...
      begin
        Index := NVJclStrings.CharPos(Quotes, NativeSingleQuote);
        if Index >= 0 then
      ...
      begin
        Index := NVJclStrings.CharPos(Quotes, NativeDoubleQuote);
        if Index >= 0 then
          SetLength(Quotes, Index - 1)
        else
      ...
      if (R[Position] = '`') then
      begin
        Index := NVJclStrings.CharPos(Quotes, '`');
        if Index >= 0 then
          SetLength(Quotes, Index - 1)
        else
          Quotes := Quotes + '`';
...
(* DELCIOfunction CreateEnvironmentBlock(const Options: TEnvironmentOptions; const AdditionalVars: TStrings): PChar;
const
...
begin
  ...
  if Result = '' then
     ...
     *)
(* DELCIO function GetShellProcessHandle: THandle;
var
  Pid: Longword;
begin
